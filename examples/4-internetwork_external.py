...
def cleanup_external_h1():
    """Clean up existing external h1 container and veth interfaces"""
    print("🧹 Cleaning up existing external h1 setup...")

    # Stop and remove external h1 container
    run_command("podman stop ukm_h1_external 2>/dev/null")
    run_command("podman rm ukm_h1_external 2>/dev/null")

    # Remove veth interfaces if they exist
    run_command("ip link delete veth-h1-ext 2>/dev/null")
    run_command("ip link delete veth-h1-int 2>/dev/null")

    print("   ✅ External h1 cleanup completed")

def setup_external_h1():
    """Setup external h1 container with proper networking"""
    print("🚀 Setting up external h1 container...")

    # Create external h1 container
    cmd = ("podman run -d --name ukm_h1_external --privileged --cap-add NET_ADMIN "
           "--cap-add SYS_ADMIN alpine:latest sleep infinity")
    success, stdout, stderr = run_command(cmd)
    if not success:
        print(f"   ❌ Failed to create external h1 container: {stderr}")
        return False

    # Install basic networking tools in container
    install_cmd = "podman exec ukm_h1_external apk add --no-cache iproute2 iputils"
    success, stdout, stderr = run_command(install_cmd, timeout=60)
    if not success:
        print(f"   ⚠️  Warning: Failed to install networking tools: {stderr}")

    print("   ✅ External h1 container created")
    return True

def create_veth_connection():
    """Create veth pair and connect external h1 to it"""
    print("🔗 Creating veth connection for external h1...")

    # Create veth pair
    cmd = "ip link add veth-h1-ext type veth peer name veth-h1-int"
    success, stdout, stderr = run_command(cmd)
    if not success:
        print(f"   ❌ Failed to create veth pair: {stderr}")
        return False

    # Get container PID for namespace operations
    pid_cmd = "podman inspect ukm_h1_external --format '{{.State.Pid}}'"
    success, stdout, stderr = run_command(pid_cmd)
    if not success:
        print(f"   ❌ Failed to get container PID: {stderr}")
        return False

    container_pid = stdout.strip()
    if not container_pid or container_pid == "0":
        print("   ❌ Invalid container PID")
        return False

    # Move veth-h1-ext to container namespace
    cmd = f"ip link set veth-h1-ext netns {container_pid}"
    success, stdout, stderr = run_command(cmd)
    if not success:
        print(f"   ❌ Failed to move veth to container: {stderr}")
        return False

    # Configure interface in container
    config_cmds = [
        "podman exec ukm_h1_external ip link set veth-h1-ext name eth0",
        "podman exec ukm_h1_external ip addr add 10.0.0.1/24 dev eth0",
        "podman exec ukm_h1_external ip link set eth0 up",
        "podman exec ukm_h1_external ip route add default via 10.0.0.254"
    ]

    for cmd in config_cmds:
        success, stdout, stderr = run_command(cmd)
        if not success:
            print(f"   ❌ Failed to configure container networking: {stderr}")
            return False

    # Bring up host side interface
    cmd = "ip link set veth-h1-int up"
    success, stdout, stderr = run_command(cmd)
    if not success:
        print(f"   ❌ Failed to bring up host veth interface: {stderr}")
        return False

    print("   ✅ veth connection established")
    print("   📍 External h1 configured with IP 10.0.0.1/24")
    return True

def connect_external_h1_to_mininet():
    """Connect external h1 veth interface to sw1 in Mininet"""
    print("🔌 Connecting external h1 to Mininet sw1...")

    # Add veth-h1-int as a port to sw1 bridge
    add_port_cmd = 'podman exec ukm_mininet ovs-vsctl add-port sw1 veth-h1-int'
    success, stdout, stderr = run_command(add_port_cmd)
    if not success:
        print(f"   ❌ Failed to add veth interface to sw1: {stderr}")
        return False

    print("   ✅ External h1 connected to sw1")
    return True

def setup_environment():
    """Setup and clean environment before testing"""
    print("🧹 Preparing Environment for 4-Internetwork Topology with External H1")
    print("======================================================================")

    # Step 1: Clean up existing external h1 and any existing Mininet processes
    print("1. Cleaning up existing external h1 and Mininet processes...")
    cleanup_external_h1()
    ...
    # Step 1.5: Setup external h1 container
    print("1.5. Setting up external h1 container...")
    if not setup_external_h1():
        print("   ❌ Failed to setup external h1 container")
        return False

    if not create_veth_connection():
        print("   ❌ Failed to create veth connection for external h1")
        return False
...
def create_4_network_topology():
    """
    Create custom 4-network topology with external h1:
    - 1 Ryu controller
    - 3 switches (sw1, sw2, sw3)
    - 6 hosts: h1 EXTERNAL CONTAINER, h2-h5 on sw2, h6 on sw3
    - h1 connected via veth pair to sw1
    """
    ...
    print("\n🌐 Creating 4-Internetwork Topology with External H1")
    print("====================================================")
    print("Network topology with 3 different subnets:")
    print("  sw1: h1 (10.0.0.1/24) - EXTERNAL CONTAINER")
    print("  sw2: h2-h5 (172.16.0.2-5/24)")
    print("  sw3: h6 (192.168.0.6/24)")
    print("Switch connections: sw1 <-> sw2 <-> sw3")

    # Create custom topology file (modified for external h1)
    topo_script = '''
...
class FourNetworkTopo(Topo):
    def build(self):
        ...
        # Note: h1 is now external - no longer created in Mininet
        # Network 1: 10.0.0.0/24 - h1 will be external container connected via veth
        ...
        # Connect internal hosts to their respective switches
        # Note: h1 connection will be added later via veth interface
...
'''
    ...
    # Create a custom Mininet script that starts topology and connects external h1
    print("\n🚀 Starting Mininet with 4-Internetwork topology...")

    mininet_script = f'''#!/usr/bin/env python3
import sys
from mininet.net import Mininet
from mininet.cli import CLI
from mininet.log import setLogLevel
from mininet.controller import RemoteController
from mininet.node import OVSSwitch
import subprocess
import time

sys.path.append('/tmp')
from fournet_topo import FourNetworkTopo
...
def main():
    ...
    controller = RemoteController('c0', ip='{controller_ip}', port=6633)
    net = Mininet(topo=FourNetworkTopo(), controller=controller, switch=OVSSwitch)
    ...
    # Connect external h1 veth interface to sw1
    print("Connecting external h1 to sw1...")
    sw1 = net.get('sw1')
    if run_cmd('ovs-vsctl add-port sw1 veth-h1-int'):
        print("✅ External h1 connected to sw1")
    else:
        print("❌ Failed to connect external h1 to sw1")

    print("\\n💡 Network ready! Try these commands:")
    print("   mininet> pingall")
    print("   mininet> h2 ping h5")
    print("   mininet> dump")
    print("   mininet> exit")
    print("\\n🔧 External h1 testing (from another terminal):")
    print("   podman exec ukm_h1_external ping -c3 10.0.0.254")
    print("   podman exec ukm_h1_external ping -c3 172.16.0.2")
    print("   podman exec ukm_h1_external ping -c3 192.168.0.6")
...
'''
    ...
    # Write the custom Mininet script
    script_cmd = f'podman exec ukm_mininet bash -c "cat > /tmp/start_fournet.py << \'EOF\'\\n{mininet_script}\\nEOF"'
    success, stdout, stderr = run_command(script_cmd)
    if not success:
        print(f"   ❌ Failed to create Mininet script: {stderr}")
        return False

    # Make script executable
    chmod_cmd = 'podman exec ukm_mininet chmod +x /tmp/start_fournet.py'
    success, stdout, stderr = run_command(chmod_cmd)
    if not success:
        print(f"   ❌ Failed to make script executable: {stderr}")
        return False

    print("   ✅ Custom Mininet script created")

    # Connect external h1 to the host veth interface first (from host side)
    print("   🔌 Preparing external h1 connection...")
    if not connect_external_h1_to_mininet():
        print("   ❌ Failed to connect external h1 - will try again inside Mininet")

    print("\n🎯 Launching interactive Mininet session with external h1...")
    ...
    # Execute the custom Mininet script
    import os
    cmd = 'podman exec -it ukm_mininet python3 /tmp/start_fournet.py'
    print(f"\\nRunning: {cmd}")
...
def main():
    """Main function"""
    print("🌐 UKMSDN 4-Network Topology Creator with External H1")
    print("===================================================")
    ...
    print("\n🎉 4-Network topology with external h1 session completed!")
